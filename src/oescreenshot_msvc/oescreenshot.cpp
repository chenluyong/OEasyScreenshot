...
void OERect::onSizeChange(int w, int h) {
    info_ = QStringLiteral("%1 × %2").arg(w).arg(h);
}
...
const QString OEScreen::getFileName(void) {
    QString file_name = QDateTime::currentDateTime().toString(QStringLiteral("CSDN博客_瓯裔_yyyy-MM-dd-HH-mm-ss"));
    return file_name;
}
...
void OEScreen::onSaveScreenOther(void) {
    ...
    QString fileName = QFileDialog::getSaveFileName(this, QStringLiteral("保存图片"), getFileName(), "JPEG Files (*.jpg)");
